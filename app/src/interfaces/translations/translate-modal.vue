...
import { getModelKey, resolveModelByKey } from '@/ai/utils/available-models';
import api from '@/api';
...
function toggleFieldSelection(fieldName: string, enabled: boolean) {
	...
	if (enabled) {
		if (!selectedFields.value.includes(fieldName)) selectedFields.value.push(fieldName);
		return;
	}
...
function toggleTargetSelection(langCode: string, enabled: boolean) {
	...
	if (enabled) {
		if (!selectedTargetLanguages.value.includes(langCode)) selectedTargetLanguages.value.push(langCode);
		return;
	}
...
const statusFields = computed(() =>
	translatableFields.value.filter((field) => selectedFields.value.includes(field.field)),
);

const statusTargetLanguages = computed(() =>
	targetLanguageOptions.value.filter((lang) => selectedTargetLanguages.value.includes(lang.value)),
);
...
function getLanguageProgressValue(langCode: string) {
	const max = statusFields.value.length;
	if (max === 0) return 0;

	const status = job.langStatuses.value[langCode]?.status;

	if (status === 'done') return 100;

	return Math.round((getLanguageProgressCount(langCode) / max) * 100);
}

function isIndeterminateLanguage(langCode: string) {
	const status = job.langStatuses.value[langCode]?.status;
	if (!isActiveStatus(status)) return false;
	return getLanguageProgressCount(langCode) === 0;
}
...
function useModelSelection() {
	...
	const configuredDefaultModelId = computed(() => settingsStore.settings?.ai_translation_default_model ?? null);

	const selectedModel = computed<AppModelDefinition | null>(() => {
		return (
			resolveModelByKey(selectedModelId.value, aiStore.models) ??
			resolveModelByKey(configuredDefaultModelId.value, aiStore.models) ??
			aiStore.models[0] ??
			null
		);
	});
	...
	function onModelSelect(model: AppModelDefinition) {
		selectedModelId.value = getModelKey(model);
	}

	function resetModelSelection() {
		selectedModelId.value = configuredDefaultModelId.value;
	}
		...
		<div class="content">
			...
			<template v-if="modalState === 'status'">
				...
				<div class="section">
					...
					<div class="selection-panel">
						<div class="selection-content status-content">
							<div
								v-for="field in statusFields"
								:key="field.field"
								class="field-row status-field-row"
								:class="`translation-status-${getFieldStatus(field.field)}`"
							>
								<span
									class="status-dot"
									:class="{ active: getFieldStatus(field.field) === 'active' }"
									aria-hidden="true"
								/>
				...
				<div class="section">
					...
					<div class="selection-panel">
						<div class="selection-content status-content">
							<div
								v-for="lang in statusTargetLanguages"
								:key="lang.value"
								class="language-row status-language-row"
								:class="`status-${job.langStatuses.value[lang.value]?.status ?? 'pending'}`"
							>
								<span class="language-label">{{ lang.text }}</span>

								<div class="language-progress">
									<span class="progress-text">
										{{ getLanguageProgressCount(lang.value) }}/{{ statusFields.length }}
									</span>

									<VProgressLinear
										:value="getLanguageProgressValue(lang.value)"
										:indeterminate="isIndeterminateLanguage(lang.value)"
										rounded
									/>
								</div>
							</div>
...
.status-dot {
	...
	&.active {
		...
		box-shadow: 0 0 0 0.1875rem color-mix(in srgb, var(--theme--primary) 14%, transparent);
	}
}
...
.status-language-row {
	...
	&.status-translating,
	&.status-retrying {
		--v-progress-linear-color: var(--theme--primary);
		--v-progress-linear-background-color: var(--theme--primary-background);
	}

	&.status-done {
		--v-progress-linear-color: var(--theme--success);
		--v-progress-linear-background-color: var(--theme--success-background);
	}

	&.status-error {
		--v-progress-linear-color: var(--theme--danger);
		--v-progress-linear-background-color: var(--theme--danger-background);
	}

	&.status-pending {
		--v-progress-linear-color: var(--theme--foreground-subdued);
		--v-progress-linear-background-color: var(--theme--background-normal);
	}
}
